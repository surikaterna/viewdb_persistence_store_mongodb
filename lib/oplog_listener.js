...
const log = LoggerFactory.getLogger('viewdb:mongodb-store:oplog-listener');
...
var OplogListener = function (mongodb, namespaceFilter) {
  ...
  this._listeners = []
  this._logThrottled = _.throttle(log.info, 1000 * 10);
};

OplogListener.prototype.tick = function (namespaceFilter) {
  log.info('[mongo-oplog] - TICK - ' + this._listeners.length + ' listeners for namespace: ' + namespaceFilter);
};

OplogListener.prototype._onError = function (err) {
  log.info('Error in oplog listener', _.get(err, 'message'));
  var self = this;
  ...
  setTimeout(function () {
    ...
    self.start().then(function () {
      log.info('[mongo-oplog] - Reconnected. Restoring ' + self._listeners.length + ' listeners');
    })
  }, retryTimeout);
};
...
OplogListener.prototype.start = function () {
  if (!this._isTailing) {
    log.info('[mongo-oplog] - connecting to oplog stream');
    this._isTailing = true;
    var self = this;
    return this._oplog.tail().then(function (stream) {
      if (stream) {
        log.info('[mongo-oplog] - connected to oplog stream');
...
OplogListener.prototype.onOperation = function (doc) {
  ...
  if (!_.has(doc, 'o.$set')) { // no need to resolve - doc already there
    ...
    observer._collection.find({ _id: id }).toArray(function (err, results) {
      ...
      if (err || !results[0]) {
        log.info('ERROR.. TODO - IMPLEMENT RETRY ?', err);
      } else {
        var res = results[0];
        _.forEach(listeners, function (listener) {
