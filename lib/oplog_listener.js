...
var OPLOG_WARN_THRESHOLD = 100;//0 * 5;
...
OplogListener.prototype.onOperation = function (doc) {
  ...
  if (oplogDiff > OPLOG_WARN_THRESHOLD) {
    this._logThrottled('[mongo-oplog] - WARN: got notified of operation with a delay of ' + oplogDiff + 'ms')
  }
  ...
  if (!_.has(doc, 'o.$set')) { // no need to resolve - doc already there
    ...
    observer._collection.find({ _id: id }).toArray(function (err, results) {
      ...
      if (resolvedSetDiff > OPLOG_WARN_THRESHOLD) {
        this._logThrottled('[mongo-oplog] - WARN: took ' + resolvedSetDiff + 'ms to resolve the document for oplog event')
      }
      if (err || !results[0]) {
      ...
      if (finishedDiff > OPLOG_WARN_THRESHOLD) {
        this._logThrottled('[mongo-oplog] - WARN: took ' + finishedDiff + 'ms to finish notifying oplog event')
      }
    });
  }
};
