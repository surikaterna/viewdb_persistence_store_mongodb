...
var Observer = function (query, queryOptions, collection, options, oplogListener) {
  ...
  this._cache = [];
  this._oplogStopHandle = undefined;

  self.loadInitial(function () {
    self._oplogStopHandle = oplogListener.listen(namespace, self._onOperation, self);
  });

  return {
    stop: function () {
      self._cache = null;
      if (self._oplogStopHandle) {
        self._oplogStopHandle();
      }
