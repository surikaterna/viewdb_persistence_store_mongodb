...
var Observer = function (query, queryOptions, collection, options) {
  ...
  self._oplog.tail().then(function () {
    self.loadInitial(function () {
      self._oplog.on('insert', self._onInsertUpdate.bind(self));
      self._oplog.on('update', self._onInsertUpdate.bind(self));
      self._oplog.on('delete', self._onRemove.bind(self));
    });
  });
...
Observer.prototype.loadInitial = function (cb) {
  ...
  this._collection._getDocuments(this._query, function (err, result) {
    if (self._options.init) {
      ...
      var old = self._cache;
      self._cache = merge(old, result, _.defaults({ comparatorId: comparator }, self._options));
    }
    cb();
  });
};
